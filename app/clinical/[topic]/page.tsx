...
import { getClinicalTopic, ClinicalFlashcard } from "@/lib/clinical";
import { getClinicalTopicProgress, clinicalFlashcardId } from "@/lib/clinical-progress";
...
function FlashcardItem({ topicSlug, index, card }: { topicSlug: string; index: number; card: ClinicalFlashcard }) {
  useStoreTick();
  const [revealed, setRevealed] = useState(false);
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  const id = clinicalFlashcardId(topicSlug, index);
...
export default function ClinicalTopicPage({ params }: { params: Promise<{ topic: string }> }) {
  const { topic: slug } = use(params);
  useStoreTick();
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  const topic = getClinicalTopic(slug);
  if (!topic) notFound();
  ...
  const progress = mounted ? getClinicalTopicProgress().find((p) => p.slug === slug) : undefined;

  return (
    <div className="space-y-6">
      <Link href="/clinical" className="text-sm text-slate-500 hover:text-slate-700">
        ← All Clinical Examination topics
      </Link>
      ...
      {/* Source badges: Macleod reference always; Russia syllabus mapping */}
        ...
        <div className="space-y-3">
          {topic.mcqs.map((m) => (
            <MCQBlock
              ...
              context={{ subject: "clinical", topic: slug, topicTitle: topic.title }}
            />
          ))}
        </div>
      </section>
      ...
      <p className="text-center text-[11px] text-slate-400">
        Original study notes aligned to Macleod — not copied from any book. For study only, not medical advice.
      </p>
    </div>
  );
}
