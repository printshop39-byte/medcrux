...
import { PATH_TOPICS, PATH_STATS, validateMCQs } from "@/lib/pathology";
import { getPathTopicProgress, getPathOverall } from "@/lib/path-progress";
...
export default function PathologyPage() {
  ...
  useEffect(() => {
    ...
    if (problems.length) console.warn("[MCQ validation] invalid Pathology MCQs:", problems);
    else console.info("[MCQ validation] all Pathology MCQs are structurally valid ✓");
  }, []);

  const progress = mounted ? getPathTopicProgress() : [];
  const overall = mounted ? getPathOverall() : null;
  const progressBySlug = new Map(progress.map((p) => [p.slug, p]));

  return (
    <div className="space-y-5">
      <SubjectSwitcher active="pathology" />
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">
          General Pathology, explained the Indian-reference way (Harsh Mohan).
          {PATH_STATS.topicCount} topics · {PATH_STATS.mcqCount} MCQs · {PATH_STATS.vivaCount} viva Qs.
        </p>
      ...
      {overall && (
        <div className="card p-5">
          <div className="flex items-center justify-between">
            <span className="section-title">Pathology readiness</span>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {PATH_TOPICS.map((t) => {
          ...
          return (
            <Link key={t.slug} href={`/pathology/${t.slug}`} className="card p-4 transition hover:shadow-md">
      ...
      <p className="text-center text-[11px] text-slate-400">
        Simplified concept notes written for revision — not copied from any textbook. For study only.
      </p>
    </div>
  );
}
