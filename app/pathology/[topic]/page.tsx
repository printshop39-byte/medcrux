...
import { getPathTopic, PathFlashcard } from "@/lib/pathology";
import { getPathTopicProgress, pathFlashcardId } from "@/lib/path-progress";
...
function FlashcardItem({ topicSlug, index, card }: { topicSlug: string; index: number; card: PathFlashcard }) {
  useStoreTick();
  const [revealed, setRevealed] = useState(false);
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  const id = pathFlashcardId(topicSlug, index);
...
export default function PathTopicPage({ params }: { params: Promise<{ topic: string }> }) {
  const { topic: slug } = use(params);
  useStoreTick();
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  const topic = getPathTopic(slug);
  if (!topic) notFound();
  ...
  const progress = mounted ? getPathTopicProgress().find((p) => p.slug === slug) : undefined;

  return (
    <div className="space-y-6">
      <Link href="/pathology" className="text-sm text-slate-500 hover:text-slate-700">
      ...
      {/* Source badges: Indian reference always; Russia syllabus if available */}
      <div className="flex flex-wrap gap-1.5">
        ...
        {topic.lectureRef ? (
          ...
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-500">
            🇷🇺 University lecture PDF pending
          </span>
        )}
        ...
        <div className="space-y-3">
          {topic.mcqs.map((m) => (
            <MCQBlock
              ...
              context={{ subject: "pathology", topic: slug, topicTitle: topic.title }}
            />
          ))}
        </div>
      </section>
